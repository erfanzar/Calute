...
export function mergeSubagentProgress(
  ...
  return compact<SubagentProgress>({
    ...
    providerProfile: payload.provider_profile ?? base.providerProfile,
    reasoningEffort: payload.reasoning_effort ?? base.reasoningEffort,
    outputTail,
...
export const subagentProgressFromSnapshot = (row: SubagentSnapshotPayload, index: number): SubagentProgress => {
  ...
  return {
    ...
    ...(row.provider_profile ? { providerProfile: row.provider_profile } : {}),
