...
export function LspSettingsOverlay({ t }: { t: Theme }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (alive.current) { setSettings(result); setSelected(0); setDraft({}); setCreating(false); setRemoving(false); setNaming(false); setEditing(false); setMessage(result.servers.length ? '' : 'No user LSP servers. F2 adds one.') }
  ...
  return <box position="absolute" left={0} top={0} width="100%" height="100%" zIndex={150} backgroundColor="#000000cc" alignItems="center" justifyContent="center">
    <Box width={size.width} height={size.height} backgroundColor={t.color.statusBg} borderStyle="round" borderColor={t.color.border} padding={1} flexDirection="column">
      <Text bold>Configuration · LSP servers</Text>
      ...
      <scrollbox style={{ flexGrow: 1, minHeight: 0 }} contentOptions={{ flexDirection: 'column' }}>
        ...
        {server ? fields.map((name, index) => compact && index !== field ? null : <Text key={name} wrap="wrap" color={field === index ? t.color.accent : t.color.text}>
          {field === index ? '› ' : '  '}{labels[index]}: {name in draft ? draft[name] === null ? '(remove)' : ['enabled', 'languageId', 'extensions', 'timeoutMs'].includes(name) ? String(draft[name]) : '(replacement entered)' : name === 'enabled' ? String(server.enabled) : name === 'languageId' ? server.languageId : name === 'extensions' ? server.extensions.join(', ') : name === 'timeoutMs' ? String(server.timeoutMs) : '(keep saved value)'}
        </Text>) : null}
      ...
      <Text wrap="wrap">{busy ? 'Saving/loading… Please wait.' : removing ? 'Y remove server · N cancel' : editing ? 'Enter apply draft · Esc cancel edit' : compact ? 'Tab/Enter edit · Ctrl+S save · F2 new · F4 remove · F5 reload · Esc' : '↑/↓ server · F2 new · F4 remove · Tab field · Enter edit · Ctrl+S save · F5 reload · Esc close'}</Text>
    </Box>
  </box>
}
