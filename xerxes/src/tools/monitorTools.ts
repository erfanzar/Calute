...
export function registerMonitorTools(registry: ToolRegistry, monitors: TerminalMonitors): void {
  const definitions: ToolDefinition[] = [
    { type: 'function', function: { name: 'monitor_terminal', description: 'Watch a terminal owned by this session. Use trigger=completion to receive its exit result and final output once (including if it just finished); match is then unnecessary. Otherwise watch future output for a case-insensitive literal match. Matching lines are deduplicated, recorded in Runs, and announced to attached users without polling the model. Set react=true only when the user requested automatic investigation or action on matches. This starts bounded background model turns; notification-only is the default. The watch expires or stops after max_events; stopping it does not kill the command.', parameters: { type: 'object', additionalProperties: false, properties: {
